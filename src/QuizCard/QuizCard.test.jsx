...
describe("QuizCard", () =>{
  ...
  beforeEach(() => {
    ...
    it('Should call the changeCurrent word function and incrememt score to be called when the correct button is clicked',() =>{
      ...
      render(<QuizCard incrementScore ={incrementScore} changeCurrentWord = {changeCurrentWord} decrementScore = {decrementScore} favoriteWords = {[{shortdef:['testWord1']},{shortdef:['testWord2']},{shortdef:['testWord3']},{shortdef:['testWord2']},{shortdef:['testWord4']}]} currentWord = {currentWord}/>)
    ...
    it('Should call the changeCurrent word function with false when wrong button is clicked',() =>{
      ...
      let incrementScore = jest.fn()
      let decrementScore =jest.fn()
      render(<QuizCard changeCurrentWord = {changeCurrentWord} incrementScore ={incrementScore} decrementScore = {decrementScore} favoriteWords = {[{shortdef:['testWord1']},{shortdef:['testWord2']},{shortdef:['testWord3']},{shortdef:['testWord2']},{shortdef:['testWord4']}]} currentWord = {currentWord}/>)
      let cardQuestion = screen.getByText("testWord1")
      userEvent.click(cardQuestion)
      expect(decrementScore).toBeCalled()
      expect(changeCurrentWord).toBeCalled()
    })
})